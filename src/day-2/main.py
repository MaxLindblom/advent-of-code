...
def get_id_if_valid_game(parsed_game: dict, bag_config: dict) -> int:
    ...
    """
    if (
        parsed_game["max_red"] > bag_config["red"]
        or parsed_game["max_green"] > bag_config["green"]
        or parsed_game["max_blue"] > bag_config["blue"]
    ):
        return 0
...
def main():
    ...
    first_result = reduce(
        lambda x, y: x + get_id_if_valid_game(parse_game(y), bag_config), games, 0
    )
